...
import { useAI, useCitationLookup } from '../../hooks/useAPI'
...
interface AdvancedDocumentViewerProps {
  ...
  onCreateCitation?: (citation: any) => void
}
...
export default function AdvancedDocumentViewer({
  ...
  const { lookupDOI, lookupISBN } = useCitationLookup()
  ...
  const handleAIAnalysis = async () => {
    ...
    try {
      const [summary, keywords] = await Promise.all([
        summarizeText(document.content),
        extractKeywords(document.content)
      ])
      
      setAIInsights({
        summary,
        keywords,
        readingTime: Math.ceil(document.content.split(' ').length / 200), // Average reading speed
        complexity: analyzeComplexity(document.content),
  ...
  const detectCitations = () => {
    ...
    const isbnRegex = /(?:ISBN[-\s]?(?:13|10)?[-\s]?)?(?:\d{1,5}[-\s]?\d{1,7}[-\s]?\d{1,7}[-\s]?[\dX])/g
    
    const dois = document.content.match(doiRegex) || []
    const isbns = document.content.match(isbnRegex) || []
    
    return { dois, isbns }
  }

  const handleAutoCreateCitation = async () => {
    const { dois, isbns } = detectCitations()
    
    if (dois.length > 0) {
      try {
        const citationData = await lookupDOI(dois[0])
        if (citationData && onCreateCitation) {
          onCreateCitation({
        ...
        console.error('Failed to create citation from DOI:', error)
      }
    } else if (isbns.length > 0) {
      try {
        const citationData = await lookupISBN(isbns[0])
        if (citationData && onCreateCitation) {
          onCreateCitation({
            type: 'book',
            ...citationData,
        ...
        console.error('Failed to create citation from ISBN:', error)
      }
    }
  }
  ...
  const renderContent = () => {
    ...
    if (showHighlights && document.highlights.length > 0) {
      ...
      sortedHighlights.forEach((highlight) => {
        ...
        content = before + 
          `<mark style="background-color: ${highlight.color}; padding: 2px 4px; border-radius: 2px;" title="${highlight.note || 'Highlight'}" data-highlight-id="${highlight.id}">${highlighted}</mark>` + 
          after
      })
    }
          ...
          <div className="p-4 border-b border-gray-200 bg-purple-50">
            ...
            <div className="space-y-3 text-sm">
              <div>
                ...
                <span className="ml-2">{aiInsights.readingTime} min</span>
              </div>
              <div>
                <span className="font-medium text-gray-700">Complexity:</span>
                <span className="ml-2">{aiInsights.complexity}</span>
              </div>
              ...
              {aiInsights.keywords && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {aiInsights.keywords.slice(0, 5).map((keyword: string) => (
                      <span
                        key={keyword}
              ...
              {aiInsights.summary && (
                <div>
                  ...
                  <p className="mt-1 text-gray-600">{aiInsights.summary}</p>
                </div>
              )}
            </div>
          </div>
